...
// WeakMap so each mounted instance has its own GL context
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSteps: { value: isMobile ? 40.0 : detailToSteps(detail) },
        ...
        uEnableMouse: { value: !isMobile && mouseInteraction },
    ...
    const pu = program.uniforms as any;
    ...
    const setSize = () => {
      ...
      const res = pu["iResolution"].value as Float32Array;
    ...
    const loop = (t: number) => {
      pu["iTime"].value = (t - t0) * 0.001;
      if (!isMobile && enableMouseRef.current) {
        currentMouse[0] = currentMouse[0] + 0.05 * (targetMouse[0] - currentMouse[0]);
        currentMouse[1] = currentMouse[1] + 0.05 * (targetMouse[1] - currentMouse[1]);
        const mouse = pu["uMouse"].value as Float32Array;
  ...
  // Sync props to uniforms on updates (without restarting GL)
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    const hRgb = hexToRgb(horizonColor);
    const wRgb = hexToRgb(waveColor);
    const cRgb = hexToRgb(crestColor);
    ...
    hc[0] = hRgb[0]; hc[1] = hRgb[1]; hc[2] = hRgb[2];
    wc[0] = wRgb[0]; wc[1] = wRgb[1]; wc[2] = wRgb[2];
    cc[0] = cRgb[0]; cc[1] = cRgb[1]; cc[2] = cRgb[2];
  }, [
